...
export class BreadcrumbService {
  ...
  private prepareBreadcrumbItem(
    ...
    const label = storeItem?.label || breadcrumb?.label;
  ...
  set(key: string, breadcrumb: string | BreadcrumbObject) {
    const label = this.extractLabel(breadcrumb);
    let updateArgs: [StoreMatcherKey, BreadcrumbDefinition];

    if (key.startsWith(ALIAS_PREFIX)) {
      updateArgs = ['alias', { label, alias: key }];
    } else if (key.includes(PATH_PARAM.PREFIX)) {
      updateArgs = ['routeRegex', { label, routeRegex: this.buildRegex(key) }];
    } else {
      updateArgs = [
        'routeLink',
        { label, routeLink: this.ensureLeadingSlash(key) },
      ];
    }
  ...
  private updateCurrentBreadcrumbs(
    ...
    const itemIndex = this.currentBreadcrumbs.findIndex((item) => {
      return key === 'routeRegex'
        ? new RegExp(breadcrumb[key]).test(item.routeLink)
