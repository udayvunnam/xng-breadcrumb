...
export class BreadcrumbService {
  ...
  private setBaseBreadcrumb() {
    ...
    if (baseConfig && baseConfig.data) {
      const breadcrumbOptions = this.getBreadcrumbOptions(baseConfig.data);
      const { alias, skip = false, info, disable = false } = breadcrumbOptions;
      let { label } = breadcrumbOptions;
  ...
  private prepareBreadcrumbItem(
    ...
    let { label, alias, skip, disable, info } = this.getFromStore(
      breadcrumb.alias,
      routeLink
    );
    let isAutoGeneratedLabel = false;
  ...
  private buildRouteRegExp(path) {
    ...
    if (path.includes(this.pathParamPrefix)) {
      ...
      const routeRegex = path.replace(
        new RegExp(this.pathParamRegexIdentifier, 'g'),
  ...
  private updateStore(breadcrumb) {
    const { breadcrumbItemIndex, storeItemIndex } = this.getBreadcrumbIndexes(
      breadcrumb
    );
  ...
  private getBreadcrumbIndexesByType(key: string, value: string) {
    ...
    if (key !== 'routeRegex') {
      ...
      breadcrumbItemIndex = this.currentBreadcrumbs.findIndex((item) =>
        new RegExp(value).test(item.routeLink)
      );
    }
  ...
  private validateArguments(pathOrAlias, breadcrumb) {
    if (pathOrAlias === null || pathOrAlias === undefined) {
      ...
      console.error(
        'Invalid second argument. Please pass a string or an Object with breadcrumb options.'
      );
      return false;
    }
    return true;
